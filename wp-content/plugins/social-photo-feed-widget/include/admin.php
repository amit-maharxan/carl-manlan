...
delete_option($pluginManagerInstance->getOptionName('proxy-check'));
$params = [];
if (isset($_GET['page'])) {
...
update_option($pluginManagerInstance->getOptionName('rate-us-feedback'), $star, false);
if ($star > 3) {
header('Location: https://wordpress.org/support/plugin/'. $pluginManagerInstance->getPluginSlug() . '/reviews/?rate='. $star .'#new-post');
}
else {
...
$proxyCheck = true;
if ($proxy->is_enabled()) {
$optName = $pluginManagerInstance->getOptionName('proxy-check');
...
<p>
<?php echo wp_kses_post(__('For some reason, the <strong>CSS</strong> file required to run the plugin was not loaded.<br />One of your plugins is probably causing the problem.', 'social-photo-feed-widget')); ?>
</p>
</div>
<?php
...
$jsFiles []= [
'id' => $id,
'url' => $pluginManagerInstance->getPluginFileUrl($file),
];
}
$jsContent = "
...
if (!isCSSExists()) {
addElement('link', '". esc_attr($pluginManagerInstance->getPluginFileUrl($assetCheckCssFile)) ."', function(success) {
loadedCount++;
if (!success) {
notLoaded.push('CSS');
}
...
<span class="ti-new-badge"><?php echo esc_html(__('new', 'social-photo-feed-widget')); ?></span>
<?php endif; ?>
</a>
<?php endforeach; ?>
...
<p>
<?php echo esc_html(__('Your site cannot download our widget templates, because of your server settings not allowing that:', 'social-photo-feed-widget')); ?><br /><a href="https://wordpress.org/support/article/editing-wp-config-php/#block-external-url-requests" target="_blank">https://wordpress.org/support/article/editing-wp-config-php/#block-external-url-requests</a><br /><br />
<strong><?php echo esc_html(__('Solution', 'social-photo-feed-widget')); ?></strong><br />
<?php echo wp_kses_post(__('a) You should define <strong>WP_HTTP_BLOCK_EXTERNAL</strong> as false', 'social-photo-feed-widget')); ?><br />
<?php echo wp_kses_post(__("b) or you should add Trustindex as an <strong>WP_ACCESSIBLE_HOSTS</strong>: \"*.trustindex.io\"", 'social-photo-feed-widget')); ?><br />
</p>
</div>
<?php endif; ?>
...
<div class="ti-box ti-notice-error ti-mb-1">
<p>
<?php echo esc_html(__('It seems you are using a proxy for HTTP requests but after a test request it returned a following error:', 'social-photo-feed-widget')); ?><br />
<strong><?php echo wp_kses_post($proxyCheck); ?></strong><br /><br />
<?php echo esc_html(__('Therefore, our plugin might not work properly. Please, contact your hosting support, they can resolve this easily.', 'social-photo-feed-widget')); ?>
</p>
<a href="<?php echo esc_url(wp_nonce_url('?page='. esc_attr(sanitize_text_field(wp_unslash($_GET['page']))) .'&tab='. esc_attr(sanitize_text_field(wp_unslash($_GET['tab']))) .'&test_proxy', 'ti-test-proxy')); ?>" class="ti-btn ti-btn-loading-on-click"><?php echo esc_html(__('Test again', 'social-photo-feed-widget')); ?></a>
</div>
<?php endif; ?>
